...
init_Cond = Generazione_coordinate_Satellite_sole('E18',deltaT,days,Start_time) % (satellite,dt,day_long)
...
% delete(gcp('nocreate'));
% n = 16; % numero di processori 2
% c = Attiva_parallelo(n);
...
% Assign back ray_b outside the parfor loop
% for ii = 1:num_iterations
%     ray_b(ii) = ray;  % Assign from cell to ray_b
% end
...
tic
          save([pat_res,'Sat_',char(filename_senza_spazi)],'Sat_b','ray_b','t','-v7');%'ray2'
toc
    end
...
% grafica_riflessioni(Sat_b(passo),ray_b{passo,1}(1,2))
% hold on;
...
%  q3 =quiver3(0,0,0,sat_sun(1,passo),sat_sun(2,passo),sat_sun(3,passo),3);
% view(-90,90-rad2deg(acos(sat_sun(3,passo))))
